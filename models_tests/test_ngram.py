...
class TestIntervalNGram( unittest.TestCase ):
   ...
   def test_string_version_9( self ):
      ...
      self.assertEqual( IntervalNGram(self.g).get_string_version( s ), 'm3 +P4 M2 -m6 P5 +A9 M-10' )
      s.set_property( 'heedQuality false' )
      self.assertEqual( IntervalNGram(self.g).get_string_version( s ), '3 +4 2 -6 5 +9 -10' )
      s.set_property( 'simpleOrCompound simple' )
      self.assertEqual( IntervalNGram(self.g).get_string_version( s ), '3 +4 2 -6 5 +2 -3' )
      s.set_property( 'heedQuality true' )
      self.assertEqual( IntervalNGram(self.g).get_string_version( s ), 'm3 +P4 M2 -m6 P5 +A2 M-3' )
   ...
   def test_retrograde_1( self ):
      self.assertTrue( IntervalNGram(self.a).retrograde() == IntervalNGram([interval.Interval(note.Note('A4'), \
            note.Note('C5')),interval.Interval(note.Note('A4'),note.Note('C5'))]) )

   def test_retrograde_2( self ):
      self.assertTrue( IntervalNGram(self.b).retrograde() == IntervalNGram([interval.Interval(note.Note('A4'), \
            note.Note('C#5')),interval.Interval(note.Note('A4'),note.Note('C5'))]) )

   def test_retrograde_3( self ):
      self.assertTrue( IntervalNGram(self.c).retrograde() == IntervalNGram([interval.Interval(note.Note('D5'), \
            note.Note('F5')),interval.Interval(note.Note('A4'),note.Note('C5'))]) )

   def test_retrograde_4( self ):
      self.assertTrue( IntervalNGram(self.d).retrograde() == IntervalNGram([interval.Interval(note.Note('D5'), \
            note.Note('F#5')),interval.Interval(note.Note('C5'),note.Note('A4'))]) )

   def test_retrograde_5( self ):
      self.assertTrue( IntervalNGram(self.e).retrograde() == IntervalNGram([interval.Interval(note.Note('E4'), \
            note.Note('G4')),interval.Interval(note.Note('A4'),note.Note('C5'))]) )

   def test_retrograde_6( self ):
      self.assertTrue( IntervalNGram(self.f).retrograde() == IntervalNGram([interval.Interval(note.Note('G#4'), \
            note.Note('E4')),interval.Interval(note.Note('A4'),note.Note('C5'))]) )

   def test_retrograde_7( self ):
      self.assertTrue( IntervalNGram(self.g).retrograde() == IntervalNGram([interval.Interval(note.Note('G##5'), \
   ...
   def test_canonical_1( self ):
      # m-3 +M2 M3
      self.assertEqual( IntervalNGram( self.d ).canonical(), 'm3 M2 M3' )

   def test_canonical_2( self ):
      # m3 -P4 m3
      self.assertEqual( IntervalNGram( self.e ).canonical(), 'm3 P4 m3' )

   def test_canonical_3( self ):
      # m3 -P4 M-3
      self.assertEqual( IntervalNGram( self.f ).canonical(), 'm3 m2 M3' )

   def test_canonical_4( self ):
      # m3 +P4 M2 -m6 P5 -m2 M-10
      self.assertEqual( IntervalNGram( self.g ).canonical(), 'm3 P4 M2 m6 P5 A9 M10' )
   ...
   def test_make_from_str_7( self ):
      ...
      self.assertRaises( NonsensicalInputError, IntervalNGram.make_from_str, \
                         '3 1 3 1' )

   def test_make_from_str_8( self ):
      # wrong number of intervals in input string
      self.assertRaises( NonsensicalInputError, IntervalNGram.make_from_str, '3 12' )
